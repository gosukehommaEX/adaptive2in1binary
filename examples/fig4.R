########################################################################################################################
## Script for Figure 4: Test size and power with different pi1, pi2, C combinations (r=1)
##
## Description:
##   This script generates Figure 4 from Homma and Yoshida (20XX), which shows
##   test size and power for each test with different combinations of pi1 (=0.2, 0.3, 0.4),
##   pi2 (=0.2, 0.3, 0.4), and C (=0.5*delta, delta, 1.5*delta) for the balanced
##   allocation ratio scenario (r=1) under alpha_2 = alpha_3 = 0.025.
##
## Design parameters:
...
# Save figures
ggsave(
  file = paste('fig4', 'eps', sep = '.'),
  plot = fig4.test.size + fig4.power +
    plot_layout(
      nrow = 1,
      widths = c(1, 1),
      guides = 'collect',
